...
void sync(struct gh_st *gh, const struct comm_st *comm)
{
    MPI_Request requests[4]; /* at most 4 send / recv */
    double *uval = gh->uval;
    int num_requests = 0;
    ...
    if (comm->lower_neighbor > -1)
    {
        ...
        MPI_Isend(&buffer, 1, MPI_DOUBLE, comm->lower_neighbor, 0,
    ...
    if (comm->upper_neighbor < comm->mpi_size)
    {
        ...
        MPI_Isend(&buffer, 1, MPI_DOUBLE, comm->upper_neighbor, 0,
                  MPI_COMM_WORLD, requests + num_requests);
        num_requests++;
    }
    ...
    if (comm->lower_neighbor > -1)
    {
        MPI_Irecv(uval, 1, MPI_DOUBLE, comm->lower_neighbor, 0, MPI_COMM_WORLD,
    ...
    if (comm->upper_neighbor < comm->mpi_size)
    {
        MPI_Irecv(uval + comm->npoints - 1, 1, MPI_DOUBLE, comm->upper_neighbor,
